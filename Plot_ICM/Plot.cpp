...
int main () {
	...
	// cout << "Input file name you want to record the results: (Without file type)" << endl;
	// cin >> Filename;
	// Filename += ".csv";
	...
	for (unsigned int k = 0; k < Core.size (); k++) {
		for (unsigned int lambda = 100; lambda < 491; lambda = lambda + 10) {
			for (unsigned int seed = 0; seed < NUMOFSEEDS; seed++) {
				string Cmd = Exec + ' ' + Topo + " Plot.txt " + to_string (NumofRequests) + ' ' + to_string (Core[k]) + ' ' + to_string (lambda * Core[k]) + " 1 " + to_string (Seed[seed]);
